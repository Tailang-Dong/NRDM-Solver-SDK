...
/*****ͨ��������Դ�ļ�, ��������. Source of GeneralPhysicsLayer, Founctions Definitions.******/
...
void Gradient_of_Vector(double** GradE, double** Vec, double** E_nei, double* E_m, int n_nei, int Dim, bool WeightedLeastSquares)
{
	...
	double phi_m;
	//3-2-2调用3-1计算每个分量的梯�?
	for (int d_attriV = 0; d_attriV < Dim; d_attriV++)//Vector attribute第d_attriV个分�?逐个分量计算
	{
		...
		for (int dcol = 0; dcol < Dim; dcol++)
			GradE[d_attriV][dcol] = Gradphi[dcol]; //赋值给张量GradE[][]的第d_attriV�?
	}
	//3-2-3释放局部变量内�?
	delete[] Gradphi;
	delete[] phi_nei;
