...
{
	double** grad_disp = new double* [Dim];//2-1-1浣嶇Щ姊害grad_disp[][]
	for (int i = 0; i < Dim; i++)
		grad_disp[i] = new double[Dim];
	Gradient_of_Vector(grad_disp, Vec, disp_nei, disp_m, n_nei, Dim, tolerance);//2-1-2璋冪敤3-2姹備綅绉绘搴?
	for (int i = 0; i < Dim; i++)
		for (int j = 0; j < Dim; j++)
			epsilon[i][j] = (grad_disp[i][j] + grad_disp[j][i]) / 2;//2-1-3鏍规嵁浣嶇Щ姊害姹傚簲鍙?
	for (int i = 0; i < Dim; i++)//2-1-4閲婃斁涓存椂鍙橀噺grad_disp[][]
		delete[] grad_disp[i];
	delete[] grad_disp;
}
...
void InForce_from_Stress_bdy(double* inforce, double** sigma_m, double* n_m, int Dim)
{
	for (int j = 0; j < Dim; j++)
	{
		inforce[j] = 0;
		for (int i = 0; i < Dim; i++)
			inforce[j] = inforce[j] - n_m[i] * sigma_m[i][j];//閫傜敤澶栨硶鍚戠畻鍐呭姏
	}
}
...
void Strain_from_Displacement(double** epsilon, double** Vec, double** disp_nei, double* disp_m, int n_nei, int Dim)
{
	...
	for (int i = 0; i < Dim; i++)
		grad_disp[i] = new double[Dim];
	Gradient_of_Vector(grad_disp, Vec, disp_nei, disp_m, n_nei, Dim);//2-1-2璋冪敤3-2姹備綅绉绘搴?
	for (int i = 0; i < Dim; i++)
		for (int j = 0; j < Dim; j++)
			epsilon[i][j] = (grad_disp[i][j] + grad_disp[j][i]) / 2;//2-1-3鏍规嵁浣嶇Щ姊害姹傚簲鍙?
	for (int i = 0; i < Dim; i++)//2-1-4閲婃斁涓存椂鍙橀噺grad_disp[][]
...
void Strain_from_Displacement(double** epsilon, double** Vec, double** Inverse_VecTVec, double** disp_nei, double* disp_m, int n_nei, int Dim)
{
	...
	for (int i = 0; i < Dim; i++)
		grad_disp[i] = new double[Dim];
	Gradient_of_Vector(grad_disp, Vec, Inverse_VecTVec, disp_nei, disp_m, n_nei, Dim);//2-1-2璋冪敤3-2姹備綅绉绘搴?
	for (int i = 0; i < Dim; i++)
		for (int j = 0; j < Dim; j++)
			epsilon[i][j] = (grad_disp[i][j] + grad_disp[j][i]) / 2;//2-1-3鏍规嵁浣嶇Щ姊害姹傚簲鍙?
	for (int i = 0; i < Dim; i++)//2-1-4閲婃斁涓存椂鍙橀噺grad_disp[][]
...
void Strain_from_Displacement(double** epsilon, double** Vec, double** disp_nei, double* disp_m, int n_nei, int Dim, bool WeightedLeastSquares)
{
	...
	for (int i = 0; i < Dim; i++)
		grad_disp[i] = new double[Dim];
	Gradient_of_Vector(grad_disp, Vec, disp_nei, disp_m, n_nei, Dim, WeightedLeastSquares);//2-1-2璋冪敤3-2姹備綅绉绘搴?
	for (int i = 0; i < Dim; i++)
		for (int j = 0; j < Dim; j++)
			epsilon[i][j] = (grad_disp[i][j] + grad_disp[j][i]) / 2;//2-1-3鏍规嵁浣嶇Щ姊害姹傚簲鍙?
	for (int i = 0; i < Dim; i++)//2-1-4閲婃斁涓存椂鍙橀噺grad_disp[][]
